...
import { getAllAvailableMonthKeys, getCurrentMonthKey } from '@/lib/month-utils';
...
export function XPDashboard({ wallet }: XPDashboardProps) {
  ...
  // Get all available month keys from Jan 2022 to current (chronological: oldest first, current last)
  const allMonthKeys = useMemo(() => getAllAvailableMonthKeys(), []);
  ...
  const [monthIndex, setMonthIndex] = useState(allMonthKeys.length - 1);

  // Reset monthIndex to current month when wallet changes (per RESEARCH.md Pitfall 4)
  useEffect(() => {
    setMonthIndex(allMonthKeys.length - 1);
  }, [wallet, allMonthKeys.length]);
  ...
  const canGoNext = monthIndex < allMonthKeys.length - 1;
